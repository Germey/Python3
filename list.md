...
```python
a = ['Google', 'Baidu', 'Taobao', 'Google'] print ('Google Index:', a.index('Google')) print ('Taobao Index:', a.index('Taobao')) print ('Runoob Index:', a.index('Runoob'))
```
...
当遇到不同字符时，会比较其ASCII码，由于小写字母的码大于大写字母，所以结果是大于，结果是True。
...
列表中存在形形色色的元素，如果要统计某个元素出现了多少次，可以使用count()方法。
